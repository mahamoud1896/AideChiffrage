...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configure(WebSecurity web) throws Exception {
        web
	       .ignoring()
	       .antMatchers("/resources/**", "/static/**","/assets/**", "/css/**", "/js/**", "/images/**", "/fonts/**", "/scss/**");
    }
	    ...
	    protected void configure(HttpSecurity http) throws Exception {
			...
			//  .antMatchers("/deleteUser/**").hasAnyAuthority("DG", "COMMERCIAL")
			 ...
			 .antMatchers("/newObjectifAnnuel/**").hasAnyAuthority("DG", "COMMERCIAL")
			  ...
			  .defaultSuccessUrl("/chiffrageTest/homePage")
			  .loginPage("/loggin")
			  ...
			  com.chiffrageTest.userInfo.CustomOAuth2User oauthUser = (com.chiffrageTest.userInfo.CustomOAuth2User)
			  authentication.getPrincipal();
			  
			  userService.processOAuthPostLogin(oauthUser.getEmail());
			  
			  response.sendRedirect("/homePage"); } }) //.defaultSuccessUrl("/list")
			  .and() .logout().logoutSuccessUrl("/loggin").permitAll() .and()
		  ...
		  @Autowired
			private com.chiffrageTest.userInfo.CustomOAuth2UserService oauthUserService;
		  
		 // @Autowired private OAuth2UserService<OAuth2UserRequest, OAuth2User> oauthUserService;
		  
		  @Autowired
			private com.chiffrageTest.userInfo.UserService userService;
